...
\end{equation}	 
	...
	\IncMargin{1em}
		...
		\KwInput{$T$,$X,K=|X|, \alpha >1/2,T \in \{1,2,... \}\cup\{\infty\}$}
		$ W = [w_{x,y}]\leftarrow 0_{K \times K} $\\
		\BlankLine
		\For{$t =1,2..,T $}{
			$U = [u_{x,y}] \leftarrow \frac{W}{W+W^T} +\sqrt{\frac{\alpha \cdot \ln(t)}{W+W^T}}$ \label{alg:rucb:line:ucb_set}\\
...
\begin{figure}[h]
	\IncMargin{1em}
		...
		\While{$|\mathcal{P}|>1$}{			
			...
			Update $W$\\
			$U = [u_{x_t, y_t}]\leftarrow \frac{W}{W+W^T} +\sqrt{\frac{\alpha}{W+W^T}}$\label{alg:savage:score_xy}\\
	...
	\IncMargin{1em}
		...
		\KwInput{$X, \mathcal{L} = \{x_1\}$, $\mathcal{S}$}
		\BlankLine
		\emph{Initialization}\\
		$p\leftarrow 1$\\
		\While{True}{
